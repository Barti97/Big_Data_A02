...
def my_model(ssc, monitoring_dir, window_duration, sliding_duration, time_step_interval):
    ...
    allstationsDStream = inputDStream.map(process_line)
    mapDStream = allstationsDStream.map(lambda item: (int(item[0]), item[1], int(item[5])))
    filterDStream = mapDStream.filter(lambda item: item[0] == 0 and item[2] == 0)
    map2DStream = filterDStream.map(lambda item: (item[1], 1))
    reducedDStream = map2DStream.reduceByKey(lambda lastNumber, nextNumber: lastNumber + nextNumber)
    updatedDStream = reducedDStream.updateStateByKey(my_state_update)
    
    sortedDStream = updatedDStream.transform( lambda item: item.sortBy(lambda item: item[1], False) )
    sortedDStream.persist(pyspark.StorageLevel.MEMORY_AND_DISK)
    sortedDStream.pprint()
    
    windowDStream = map2DStream.reduceByKeyAndWindow(lambda x, y: x + y,
                                                     lambda x, y: x - y,
                                                     window_duration * time_step_interval,
                                                     sliding_duration * time_step_interval
                                                     )
    
    sorted2DStream = windowDStream.transform( lambda item: item.sortBy(lambda item: item[1], False) )
    sorted2DStream.persist(pyspark.StorageLevel.MEMORY_AND_DISK)
    sorted2DStream.pprint()
